...
namespace TestSelenium_BDCLPM.Category
{
    public class AddTop
    {
        ...
        private string categoryTop, showOnMenu;
        ...
        public void Setup()
        {
            ...
            // Click Top Cate
            driver.FindElement(By.XPath("/html/body/div/aside/div/section/ul/li[3]/ul/li[5]/a")).Click();
            Thread.Sleep(2000);
        ...
        public void AddCate()
        {
            ...
            var worksheet = package.Workbook.Worksheets["Add_TopCate"];
            ...
            for (int row = 3; row <= worksheet.Dimension.End.Row; row++)  // Bắt đầu từ dòng 3
            {
                ...
                showOnMenu = worksheet.Cells[row, 7].Text;
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(categoryTop))
                        driver.FindElement(By.Name("tcat_name")).SendKeys(categoryTop);
                    Thread.Sleep(1000);

                    // Chọn trạng thái "showonmenu"
                    IWebElement dropdown4 = driver.FindElement(By.Name("show_on_menu"));
                    SelectElement select = new SelectElement(dropdown4);
                    select.SelectByValue(showOnMenu);
                    Thread.Sleep(1000);
