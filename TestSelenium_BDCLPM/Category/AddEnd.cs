...
namespace TestSelenium_BDCLPM.Category
{
    public class AddEnd
    {
        ...
        private string categoryTop, categoryMid, categoryEnd;
        ...
        public void Setup()
        {
            ...
            // Click End Cate
            driver.FindElement(By.XPath("/html/body/div/aside/div/section/ul/li[3]/ul/li[7]/a")).Click();
            Thread.Sleep(2000);
        ...
        public void AddCate()
        {
            ...
            var worksheet = package.Workbook.Worksheets["Add_EndCate"];
            ...
            for (int row = 3; row <= worksheet.Dimension.End.Row; row++)  // Bắt đầu từ dòng 3
            {
                ...
                string element = worksheet.Cells[row, 9].Text; // XPath cần kiểm tra
                string expectedResult = worksheet.Cells[row, 12].Text;
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(categoryEnd))
                        driver.FindElement(By.Name("ecat_name")).SendKeys(categoryEnd);
                    Thread.Sleep(2000);
                    ...
                    if (!string.IsNullOrEmpty(element))
                    {
                        try
                        {
                            ...
                            if (elementAfterAdd.Displayed)
                            {
                                ...
                                worksheet.Cells[row, 12].Value = "Passed";
                            }
                            else
                            {
                                Console.WriteLine($"Test Case {testCaseId}: Failed. Element is not displayed.");
                                worksheet.Cells[row, 12].Value = "Failed";
                            }
                        }
